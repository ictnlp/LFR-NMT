...
class Trainer_kd(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, model_teacher, criterion, quantizer=None):
        ...
        for p in self._model_teacher.parameters():
        ...
        if not self.is_fsdp:
            if cfg.common.fp16:
                ...
                self._model_teacher = self._model_teacher.half()
            elif cfg.common.bf16:
                self._criterion = self._criterion.to(dtype=torch.bfloat16)
                self._model = self._model.to(dtype=torch.bfloat16)
                self._model_teacher = self._model_teacher.to(dtype=torch.bfloat16)
            elif cfg.common.amp:
                self._amp_retries = 0
        if (
        ...
        # the teacher model doesn't need wrapper, so it should be moved to the device.
        self._model_teacher = self._model_teacher.to(device=self.device)
        ...
        self._wrapped_model_teacher = None
        self._ema = None
    ...
    @property
    def model_teacher(self):
        if self._wrapped_model_teacher is None:
            if False:
                # if self.use_distributed_wrapper:
                self._wrapped_model_teacher = models.DistributedFairseqModel(
                    self.cfg.distributed_training,
                    self._model_teacher,
                    process_group=self.data_parallel_process_group,
                    device=self.device,
                ...
                self._wrapped_model_teacher = self._model_teacher
        return self._wrapped_model_teacher
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            if load_on_all_ranks or self.data_parallel_rank == 0:
                ...
                # load teacher model
                state_teacher = checkpoint_utils.load_checkpoint_to_cpu(
                    #filename + '.teacher', load_on_all_ranks=load_on_all_ranks
                    filename, load_on_all_ranks=load_on_all_ranks
                )
            ...
            if is_distributed and not load_on_all_ranks:
                ...
                state_teacher = distributed_utils.broadcast_object(
                    state_teacher,
                    src_rank=0,
                    group=self.data_parallel_process_group,
                    dist_device=self.device,
            ...
            try:
                ...
                #self.model.register_par_mask()
                self.model_teacher.load_state_dict(
                    state_teacher["model"], strict=True, model_cfg=self.cfg.model
                )
